...
static u8 BaudRate8CLKArr[13]={207,103,51,34,25,16,12,8,6,3,1,1,0};

static u8 BaudRate16CLKArr[13]={416,207,103,68,51,34,25,16,12,8,3,3,0};

void UART_Init(void)
{
	ParityMode_type parity=PARITY_MODE;
	DataBits_type dataType=Data_BITS_TYPE;
	
	//boad rate
	UBRRL=BaudRate8CLKArr[BAUD_RATE];
	...
	#if STOP_BIT_TYPE==STOP_BIT_1bit
	CLR_BIT(UCSRC,USBS);
	#elif STOP_BIT_TYPE==STOP_BIT_2bit
	SET_BIT(UCSRC,USBS);
	#endif
